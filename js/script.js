...
var apiURL = 'https://app.ticketmaster.com/discovery/v2/events.json?sort=random&countryCode=GB&apikey=';
...
function displayMatches(matches) {
  ...
  if (!matches) {
    ...
    for (var matchObj of matches) {                              // Loop through the the events for a city and display them on the screen with their titles, date, time, venue and background image as shown in the var=events (console.log(events) to see the various properties to pass in to view title and the rest). Styling of the background image is done in CSS
      itemWrapper.append(`         
...
function getEventData(event) {
  ...
  if (keyCode === 13 && searchText) {    // Checking to see if the key pressed is the enter key and if some text is typed in the input box
    ...
    $.get(proxyURL + encodeURIComponent(apiURL + apiKey + `&city=${searchText}`))
      .then(function (data) {
